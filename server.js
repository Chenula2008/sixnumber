...
async function startServer() {
    // 🚨 CRITICAL FIX: Manually resolve Gmail's IPv4 address to completely bypass Render's IPv6 block
    let smtpHost = 'smtp.gmail.com';
    try {
        const addresses = await resolve4('smtp.gmail.com');
        if (addresses.length > 0) {
            smtpHost = addresses[0];
            console.log('✅ Resolved Gmail IPv4:', smtpHost);
        }
    } catch (err) {
        console.error('❌ IPv4 resolve failed, using hostname:', err);
    }

    // Setup Nodemailer with the resolved IPv4 address
    const transporter = nodemailer.createTransport({
        ...
        tls: {
            servername: 'smtp.gmail.com' // Required when using an IP so Gmail's SSL certificate validates correctly
        }
    });

    // 🚀 GMAIL HELPER FUNCTION (Includes Plain Text to prevent Spam folders)
    async function sendEmail(to, subject, html, plainText, replyToEmail = null) {
        const mailOptions = {
            from: `"SixNumber" <${process.env.EMAIL_USER}>`,
            to: to,
            subject: subject,
            text: plainText, // 🚨 Plain text version prevents Spam folder
            html: html
        };
        if (replyToEmail) {
    ...
    app.post('/signup', async (req, res) => {
        ...
        try {
            ...
            sendEmail(email, 'Please verify your email for SixNumber', emailHtml, plainText);
    ...
    app.post('/api/contact', authMiddleware, async (req, res) => {
        ...
        try {
            ...
            // 🚀 SEND SUPPORT EMAIL VIA GMAIL
            sendEmail(process.env.EMAIL_USER, `📩 New Support Message: ${subject}`, contactHtml, contactPlainText, user.email);
